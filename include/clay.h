...
// <my stuff :3>
#define Clay_GetColor(hexvalue) ((Clay_Color) {  \
  .r = (hexvalue >> 24)  & 0xFF,                 \
  .g = (hexvalue >> 16)  & 0xFF,                 \
  .b = (hexvalue >> 8)   & 0xFF,                 \
  .a = hexvalue & 0xFF                           \
...
#if !defined(CLAY__STRING_LENGTH)
# define CLAY__STRING_LENGTH(s) (strlen(s))
#endif
...
CLAY__TYPEDEF(Clay_TextElementConfigSubText, struct {
    bool use;
    int pos,length;
});
...
struct Clay_TextElementConfig {
    ...
    Clay_TextElementConfigSubText subText;
