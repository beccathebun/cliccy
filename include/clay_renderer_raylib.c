...
#define RLV2_TO_CLAYV2(vector) (Clay_Vector2) { .x = vector.x, .y = vector.y }
    ...
    for (int j = 0; j < renderCommands.length; j++)
    {
        ...
        switch (renderCommand->commandType)
        {
            case CLAY_RENDER_COMMAND_TYPE_TEXT: {
                ...
                const char *final = 
                  renderCommand->config.textElementConfig->subText.use 
                  ? TextSubtext(cloned, 
                  renderCommand->config.textElementConfig->subText.pos, 
                  renderCommand->config.textElementConfig->subText.length) 
                  : cloned;
                DrawTextEx(fontToUse, final, (Vector2){boundingBox.x, boundingBox.y}, (float)renderCommand->config.textElementConfig->fontSize, (float)renderCommand->config.textElementConfig->letterSpacing, CLAY_COLOR_TO_RAYLIB_COLOR(renderCommand->config.textElementConfig->textColor));
                free(cloned);
                break;
            }
            ...
            case CLAY_RENDER_COMMAND_TYPE_RECTANGLE: {
                ...
                if (config->cornerRadius.topLeft > 0) {
                    ...
                    if(config->lineThick > EPSILON) {
                      DrawRectangleRoundedLinesEx((Rectangle) { boundingBox.x, boundingBox.y, boundingBox.width, boundingBox.height }, radius, 8, config->lineThick, CLAY_COLOR_TO_RAYLIB_COLOR(config->color));
                    } else {
                ...
                } else if(config->lineThick > EPSILON) {
                  DrawRectangleLinesEx((Rectangle) { boundingBox.x, boundingBox.y, boundingBox.width, boundingBox.height }, config->lineThick, CLAY_COLOR_TO_RAYLIB_COLOR(config->color));
                } else {
